...
class WhereTest < Minitest::Test
  ...
  def test_where_with_mutliple_partial_results
    assert_equal [@boris, @charles, @wolf], @fixtures.where(:quote => /you/)
  end
  ...
  def test_where_with_with_both_multiple_criteria_and_results
    assert_equal [@boris, @wolf], @fixtures.where(:rank => 4, :quote => /I/)
  end
  ...
  def test_where_with_chain_calls_and_multiple_results
    assert_equal [@boris, @wolf], @fixtures.where(:quote => /if/i).where(:rank => 4)
  end

  def test_where_returns_an_empty_array_given_bad_arguments
    assert_equal [], @fixtures.where(:quote => 'You will never find out who said this.')
  end
end
